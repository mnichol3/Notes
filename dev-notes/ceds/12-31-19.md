...
## Issue 1: Error in `CEDS_version_comparison.R`
...
## Issue 2: Error in `CEDS_version_comparison.R` (again)
...
* Re-run `CEDS_version_comparison.R` on branch `frozen_ems`
...
## Issue 4: Frozen ENE sector

The CEDS comparison plots produced by `CEDS_version_comparison.R` show that emissions relating to the sector `ENE`, aggregate region `Africa, Central and South America, and Middle East` (aka `ACSME`), and fuel `process` have a hard spike around the year 2010


### Steps to solve

* Determine which CEDS sectors comprise the `ENE` sector on the plots
  * Read `CEDS-dev/input/emissions-inventories/RCP/RCP_CEDS_sector_map.csv` into a Pandas DataFrame:
    ```
    > import pandas as pd
    > from os.path import join
    > fpath = join("code", "CEDS-dev", "input", "emissions-inventories", "RCP")
    > fname = "RCP_CEDS_sector_map.csv"
    > df = pd.read_csv(join(fpath, fname), sep=',', header=0)
    > df.head()
                                CEDS  RCP
    0           1A1a_Heat-production  ENE
    1        1A1a_Electricity-public  ENE
    2  1A1a_Electricity-autoproducer  ENE
    3     1A1bc_Other-transformation  ENE
    4       1A2a_Ind-Comb-Iron-steel  IND
    
    >
    ```
    
  * Get a subset of the DataFrame where values in the `RCP` column equal `ENE`:
    ```
    > df2 = df.loc[df['RCP'] == 'ENE']
    > df2
                                 CEDS  RCP
    0            1A1a_Heat-production  ENE
    1         1A1a_Electricity-public  ENE
    2   1A1a_Electricity-autoproducer  ENE
    3      1A1bc_Other-transformation  ENE
    28       1B1_Fugitive-solid-fuels  ENE
    29      1B2_Fugitive-petr-and-gas  ENE
    30     1B2d_Fugitive-other-energy  ENE
    ```
    
  * Print the values of the `CEDS` column:
    ```
    > for sec in df_ene['CEDS'].values.tolist():
    ...     print(sec)
    ...
  ...
  * Cross-referencing these CEDS sectors with data contained in `CEDS-dev/input/mappings/Master_Fuel_Sector_List.xlsx` yields:
    ```
    1A1a_Heat-production -----------> comb
    1A1a_Electricity-public --------> comb
    1A1a_Electricity-autoproducer --> comb
    1A1bc_Other-transformation -----> NC
    1B1_Fugitive-solid-fuels -------> NC
    1B2_Fugitive-petr-and-gas ------> NC
    1B2d_Fugitive-other-energy -----> NC
    ```
